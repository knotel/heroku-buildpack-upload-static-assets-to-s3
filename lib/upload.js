...
glob(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/**', {}, function(error, files) {
    ...
    async.eachLimit(files, 16, function(file, callback) {
        ...
        if (stat.isDirectory()) {
          let key = path.join(STATIC_PATH, file.replace(PUBLIC_ASSETS_SOURCE_DIRECTORY, ''))
          if (key.lastIndexOf('/') < key.length - 1) {
            key = key + '/'
          }
          // We don't need to create the root directory
          if (key.length === 1) return callback(null)
          const params = {
            ACL: 'public-read',
            Key: key,
            Bucket: AWS_STATIC_BUCKET_NAME,
          }
          console.log('Creating dir: ' + params.Key + (params.ContentEncoding ? ' (' + params.ContentEncoding + ')': ''))
